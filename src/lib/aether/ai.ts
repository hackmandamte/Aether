...
/** Safety bound for model-emitted tool calls (not a UX feature limit). */
...
export const speakAether = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return { ok: false as const, error: "Couldn't synthesize speech." };
  });
...
export const hearAether = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const provider = await getProvider();
    if (!provider.key) {
      return { ok: false as const, error: "Speech recognition is not configured." };
    }
    ...
    if (bytes.byteLength < 200 || bytes.byteLength > MAX_AUDIO_BYTES) {
      return { ok: false as const, error: "Audio was empty or too large." };
    }
    ...
    if (!res) return { ok: false as const, error: "Couldn't reach speech recognition." };
    if (!res.ok) {
    ...
    return { ok: true as const, text: text.slice(0, 2000) };
  });
