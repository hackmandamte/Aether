...
public class AetherBridge {
    ...
    public JSONObject execute(JSONObject obj) {
        try {
            ...
            String value = clip(obj.optString("value", ""), action.equals("type_text") ? 500 : 60);
            switch (action) {
                case "flashlight_on": return torch(true);
                case "flashlight_off": return torch(false);
                case "volume": return volume(parseInt(value.isEmpty() ? target : value, 11));
                case "brightness": return brightness(parseInt(value.isEmpty() ? target : value, 70));
                case "call": return call(target);
                case "sms": return sms(target, extra.isEmpty() ? value : extra);
                case "open_app": return openApp(target);
                case "camera": return camera();
                case "alarm": return alarm(value.isEmpty() ? target : value, extra);
                case "timer": return timer(parseInt(value.isEmpty() ? target : value, 60), extra);
                case "lock": return global("lock");
                case "home": return global("home");
                case "back": return global("back");
                case "wifi": return toggleWifi(value.isEmpty() ? "on" : value);
                case "bluetooth": return toggleBluetooth(value.isEmpty() ? "on" : value);
                case "hotspot": return toggleHotspot(value.isEmpty() ? "on" : value);
                case "type_text": return typeIntoFocused(value.isEmpty() ? target : value);
                case "navigate": return navigate(target);
                case "location": return location();
                case "search_web": return searchWeb(target.isEmpty() ? extra : target);
                case "open_url": return openWebUrl(target.isEmpty() ? extra : target);
                default: return result(false, "Unknown action");
            }
        } catch (Exception e) {
    ...
    private JSONObject openApp(String name) {
        ...
        if (pkg == null) {
            for (Map.Entry<String, String> e : APPS.entrySet()) {
                if (key.contains(e.getKey()) || e.getKey().contains(key)) { pkg = e.getValue(); break; }
            }
        }
        if (pkg == null) {
            ...
            String bestPkg = null; int bestScore = 0;
            for (ResolveInfo ri : launchers) {
                ...
                if (score > bestScore) { bestScore = score; bestPkg = ri.activityInfo.packageName; }
            }
    ...
    private JSONObject camera() {
        ...
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        try { activity.startActivity(i); return result(true, "Camera."); }
        catch (Exception e) { return result(false, "Couldn't open the camera."); }
    }
    ...
    private JSONObject timer(int seconds, String label) {
        ...
        i.putExtra(AlarmClock.EXTRA_SKIP_UI, true);
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        try { activity.startActivity(i); return result(true, "Timer started."); }
    ...
    private JSONObject searchWeb(String query) {
        ...
        Intent i = new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.google.com/search?q=" + Uri.encode(query.trim())));
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        activity.startActivity(i);
    ...
    private JSONObject toggleWifi(String value) {
        boolean wantOn = !"off".equalsIgnoreCase(value);
        try {
            android.net.wifi.WifiManager wm = (android.net.wifi.WifiManager)
                    activity.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
            if (wm != null) {
                @SuppressWarnings("deprecation")
                boolean ok = wm.setWifiEnabled(wantOn);
                if (ok || wm.isWifiEnabled() == wantOn) {
                    return result(true, wantOn ? "Wi-Fi on." : "Wi-Fi off.");
                }
            }
        } catch (Exception ignored) {}
        return openSettings(Settings.ACTION_WIFI_SETTINGS, "Wi-Fi");
    }
    private JSONObject toggleBluetooth(String value) {
        boolean wantOn = !"off".equalsIgnoreCase(value);
        try {
            android.bluetooth.BluetoothAdapter ba = android.bluetooth.BluetoothAdapter.getDefaultAdapter();
            if (ba != null) {
                boolean ok = wantOn ? ba.enable() : ba.disable();
                if (ok) return result(true, wantOn ? "Bluetooth on." : "Bluetooth off.");
            }
        } catch (Exception ignored) {}
        return openSettings(Settings.ACTION_BLUETOOTH_SETTINGS, "Bluetooth");
    }
    private JSONObject toggleHotspot(String value) {
        try {
            Intent i = new Intent(Intent.ACTION_MAIN, null);
            i.setClassName("com.android.settings", "com.android.settings.TetherSettings");
            i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            activity.startActivity(i);
            return result(true, "Opened hotspot panel.");
        } catch (Exception e) {
            return openSettings(Settings.ACTION_WIRELESS_SETTINGS, "Hotspot");
        }
    }
    private JSONObject typeIntoFocused(String text) {
        if (text == null || text.trim().isEmpty()) {
            return result(false, "Nothing to type.");
        }
        boolean ok = AetherAccessibilityService.typeText(text.trim());
        if (ok) return result(true, "Typed.");
        return result(false, "Enable ETA in Settings \u2192 Accessibility to type into other apps.");
    }
